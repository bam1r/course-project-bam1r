...
from .security import CurrentUser, get_current_user, require_admin, ensure_owner_or_admin
from .models.asset import AssetOut, AssetCreate
from .models.checkout import CheckoutOut, CheckoutCreate, CheckoutStatus, can_transition
from .models.user import UserOut, UserCreate, UserRole
...
_DB: Dict[str, List[Dict]] = {"users": [], "assets": [], "checkouts": [], "equipment": []}
...
def _has_active_checkout(asset_id: int) -> bool:
    ...
    for checkout in _DB["checkouts"]:
        if (checkout.get("asset_id") == asset_id and
                checkout.get("status") in active_statuses):
            return True
...
def create_user(user: UserCreate, current_user: CurrentUser = Depends(get_current_user)):
    require_admin(current_user)
    for u in _DB["users"]:
...
def update_user(user_id: int, user: UserCreate, current_user: CurrentUser = Depends(get_current_user)):
    require_admin(current_user)
...
def create_asset(asset: AssetCreate, current_user: CurrentUser = Depends(get_current_user)):
    require_admin(current_user)
...
def update_asset(asset_id: int, asset: AssetCreate, current_user: CurrentUser = Depends(get_current_user)):
    require_admin(current_user)
...
def get_checkout(checkout_id: int, current_user: CurrentUser = Depends(get_current_user)):
...
def create_checkout(checkout: CheckoutCreate, current_user: CurrentUser = Depends(get_current_user)):
    _get_record("assets", checkout.asset_id, "Asset not found")

    if _has_active_checkout(checkout.asset_id):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=f"Asset is already checked out"
        )
...
def update_checkout(checkout_id: int, checkout: CheckoutCreate, current_user: CurrentUser = Depends(get_current_user)):
...
def delete_checkout(checkout_id: int, current_user: CurrentUser = Depends(get_current_user)):
    checkout = _get_record("checkouts", checkout_id, "Checkout not found")
    ensure_owner_or_admin(checkout["owner_id"], current_user)
